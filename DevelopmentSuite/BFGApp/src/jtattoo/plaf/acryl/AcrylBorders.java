...
public class AcrylBorders extends BaseBorders {
    ...
    public static class ButtonBorder implements Border, UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            Color backColor = c.getParent().getBackground();
            Object savedRederingHint = g2D.getRenderingHint(RenderingHints.KEY_ANTIALIASING);
            g2D.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            // top
            g2D.setColor(ColorHelper.darker(backColor, 10));
            g2D.drawLine(x, y + 2, x + 2, y);
            g2D.drawLine(x + 2, y, w - 2, y);
            g2D.drawLine(w - 2, y, w, y + 2);
            //left
            g2D.setColor(ColorHelper.darker(backColor, 5));
            g2D.drawLine(x, y + 2, x, h - 2);
            ///right
            g2D.drawLine(w, y + 2, w, h - 2);
            //bottom
            g2D.setColor(ColorHelper.brighter(backColor, 20));
            g2D.drawLine(x, h - 2, x + 2, h);
            g2D.drawLine(w, h - 2, w - 2, h);
            g2D.setColor(ColorHelper.brighter(backColor, 60));
            g2D.drawLine(x + 2, h, w - 2, h);
            if (!model.isPressed() && !model.isArmed() && button.isDefaultCapable() && button.isDefaultButton() && JTattooUtilities.isFrameActive(button)) {
                g.setColor(AbstractLookAndFeel.getTheme().getControlColorLight());
                g2D.drawLine(x + 2, y + 3, x + 2, h - 2);
                g2D.drawLine(w - 2, y + 3, w - 2, h - 2);
                g2D.drawLine(x + 3, y + 2, x + 3, h - 1);
                g2D.drawLine(w - 3, y + 2, w - 3, h - 1);
            }
            ...
            g.drawLine(0, h, 1, h + 1);
            g.drawLine(w, h, w + 1, h + 1);
            g.drawLine(w, h + 1, w + 1, h + 1);
        }
    ...
    public static class ToggleButtonBorder implements Border, UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            if (model.isEnabled()) {
                ...
                Color frameLoColor1 = ColorHelper.brighter(AcrylLookAndFeel.getFrameColor(), 65);
                ...
                if ((model.isPressed() && model.isArmed()) || model.isSelected()) {
                    ...
                    JTattooUtilities.draw3DBorder(g, frameLoColor1, frameHiColor1, x + 2, y + 2, w - 4, h - 4);
                } else {
    ...
    public static class InternalFrameBorder extends BaseInternalFrameBorder {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            Color titleColor = AcrylLookAndFeel.getWindowInactiveTitleBackgroundColor();
            Color borderColor = AcrylLookAndFeel.getWindowInactiveTitleColorDark();
            Color frameColor = ColorHelper.darker(AcrylLookAndFeel.getWindowInactiveBorderColor(), 10);
            if (active) {
                titleColor = AcrylLookAndFeel.getWindowTitleColorLight();//ColorHelper.brighter(AcrylLookAndFeel.getWindowTitleColorLight(), 20);
