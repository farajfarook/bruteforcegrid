    ...
    protected Color getGapColor(int tabIndex) {
        if ((tabIndex >= 0) && (tabIndex < tabPane.getTabCount())) {
            ...
            if ((backColor instanceof UIResource)) {
                ...
                if (tabIndex == tabPane.getSelectedIndex()) {
                    if (tabPane.getTabPlacement() == BOTTOM) {
                        return ColorHelper.brighter(backColor, 50);
                    } else {
                        return backColor;
                    }
                } else {
...
//            ex.printStackTrace(outStream);
    ...
    protected void paintBottomTabBorder(int tabIndex, Graphics g, int x1, int y1, int x2, int y2, boolean isSelected) {
        ...
        g.drawLine(x1 + 1, y2 - GAP - 1, x1 + GAP + 1, y2 - 1);
    ...
    protected void paintRightTabBorder(int tabIndex, Graphics g, int x1, int y1, int x2, int y2, boolean isSelected) {
        ...
        if (isSelected && (tabIndex != lastIndex)) {
            g.drawLine(x2 - GAP, y2, x1, y2);
        } else {
            g.drawLine(x2, y2, x1, y2);
        }
        g.setColor(hiColor);
        g.drawLine(x1 + 1, y1 + 1, x2 - GAP, y1 + 1);
        g.drawLine(x2 - GAP - 1, y1 + 1, x2 - 1, y1 + GAP + 1);
        // paint gap
        int gapTabIndex = getTabAtLocation(x1 + 2, y1 - 2);
        Color gapColor = getGapColor(gapTabIndex);
        g.setColor(gapColor);
    ...
    protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected) {
        ...
        switch (tabPlacement) {
            ...
            case RIGHT:
                if (isSelected) {
                    JTattooUtilities.fillHorGradient(g, colorArr, x, y + 1, w, h - 1);
                } else {
                    JTattooUtilities.fillHorGradient(g, colorArr, x + 1, y + 1, w - 1, h - 1);
                }
                break;
    ...
    public class ArrowButton extends JButton implements SwingConstants {
        ...
        public ArrowButton(int direction) {
            ...
            if (simpleButtonBorder) {
                setBorder(BorderFactory.createEtchedBorder(AbstractLookAndFeel.getTheme().getControlHighlightColor(), AbstractLookAndFeel.getTheme().getControlDarkShadowColor()));
            }
        }
    ...
    public class PropertyChangeHandler implements PropertyChangeListener {
        ...
        public void propertyChange(PropertyChangeEvent e) {
            ...
            if ("mnemonicAt".equals(name)) {
                ...
                try {
                    Component c = tabPane.getTabComponentAt(((Integer) e.getNewValue()).intValue());
                    if (c != null) {
                        if (tabContainer == null) {
                            installTabContainer();
                        } else {
